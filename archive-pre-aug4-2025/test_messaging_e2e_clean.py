...
def test_clean_message():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"[SUCCESS] Clean message sent successfully!")
            print(f"Filtered content: '{result.get('filtered_content', 'N/A')}'")
            print(f"Content was filtered: {result.get('content_filtered', False)}")
            print(f"Filter reasons: {result.get('filter_reasons', [])}")
            return True
...
def test_broadcast_message():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"[SUCCESS] Broadcast sent successfully!")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if health_response.status_code == 200:
            ...
            print(f"[HEALTH] Database: {health_data.get('database', 'unknown')}")
        else:
            print(f"[ERROR] Health check failed: {health_response.status_code}")
            exit(1)
    except Exception as e:
        print(f"[ERROR] Cannot connect to messaging API: {e}")
        exit(1)
    ...
    if passed == total:
        print(f"[SUCCESS] All {total} tests PASSED! Messaging system is working correctly.")
    else:
        print(f"[PARTIAL] {passed}/{total} tests passed. Some issues detected.")
