#!/usr/bin/env python3
"""
Test a real conversation through the API on port 8008
"""
import asyncio
import requests
import json
...
async def test_real_conversation_flow():
    """Test real conversation through API on port 8008"""
    
    print("Testing real conversation flow through API on port 8008...")
    ...
    print("1. Sending message to CIA API on port 8008...")
    try:
        response = requests.post(
            "http://localhost:8008/api/cia/chat",  # Using port 8008
            json=message_data,
            timeout=30
        )
    ...
    try:
        history_response = requests.get(
            f"http://localhost:8008/api/cia/conversation/{session_id}",  # Using port 8008
            timeout=10
        )
        ...
        if history_response.status_code == 200:
            ...
            if history_data.get('success') and history_data.get('messages'):
                ...
                if message_count >= 2:  # Should have user message + assistant response
                    ...
                    print("   ✅ CONVERSATIONS ARE NOW BEING SAVED TO DATABASE!")
                else:
