#!/usr/bin/env python3
"""
Test database saving with correct UUID
"""
import asyncio
import json
import sys
...
async def test_database_save_fixed():
    """Test database saving with correct UUID"""
    
    print("Testing database conversation saving with correct UUID...")
    ...
    try:
        ...
        # Create test state with proper UUID
        session_id = f"debug_session_{int(datetime.now().timestamp())}"
        user_id = "0912f528-924c-4a7c-8b70-2708b3f5f227"  # Valid UUID from database
        ...
        print(f"2. Created test state with {len(test_state['messages'])} messages and UUID: {user_id}")
        ...
        try:
            ...
            print(f"   SUCCESS: Save method worked! Result: {result}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success') and data.get('messages'):
                    ...
                    print("   ✅ FINAL RESULT: Database saving and API retrieval working!")
                else:
...
if __name__ == "__main__":
    asyncio.run(test_database_save_fixed())