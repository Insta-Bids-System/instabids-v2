...
def get_bid_card_status(bid_card_id):
    """Get current bid card status"""
    response = requests.get(f"{BASE_URL}/api/cia/potential-bid-cards/{bid_card_id}")
    if response.status_code == 200:
...
def chat_with_cia(message, turn_number):
    ...
    try:
        ...
        print(f"⏱️  Response time: {response_time:.2f}s, Chunks: {chunk_count}")
...
def check_bid_card_after_turn(turn_number):
    ...
    if bid_card:
        ...
        if bid_card.get('bid_card_preview'):
            ...
            print(f"      Description: {preview.get('description', 'N/A')[:100]}...")
...
async def main():
    ...
    if final_bid_card:
        ...
        if final_bid_card['ready_for_conversion']:
            ...
            print(f"\n⚠️  Still missing: {final_bid_card['missing_fields']}")
