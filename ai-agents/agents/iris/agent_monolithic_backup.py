...
# Note: uuid and datetime already imported above
...
3. Show the user you're actively helping, not just chatting"""
    ...
    def get_complete_context(self, user_id: str, session_id: str) -> Dict[str, Any]:
        """Get complete context with conversation history and basic data - BALANCED performance"""
        try:
            ...
            # Get basic inspiration context (lightweight)
            inspiration_context = self._get_inspiration_context(user_id, limit=5)
            
            # Get recent image context from memory
            recent_images = self._get_recent_image_context(user_id, session_id)
            
            return {
                "inspiration": inspiration_context,
                "property": {"properties": []},  # Still disabled for performance
                "trade_projects": {"trade_groups_by_property": {}, "total_trade_projects": 0},  # Still disabled for performance
    ...
    def _get_inspiration_context(self, user_id: str, limit: int = 5) -> Dict[str, Any]:
        """Get basic inspiration boards for user - lightweight query"""
        try:
            boards_result = db.client.table("inspiration_boards")\
                .select("id,title,room_type,status,created_at")\
                .eq("user_id", user_id)\
            ...
            return {
                "inspiration_boards": boards_result.data or [],
                "total_boards": len(boards_result.data or [])
            }
        except Exception as e:
            logger.warning(f"Could not get inspiration context: {e}")
            return {"inspiration_boards": [], "total_boards": 0}

    def _get_recent_image_context(self, user_id: str, session_id: str) -> List[Dict[str, Any]]:
        """Get recent image context from unified memory"""
        try:
            memory_result = db.client.table("unified_conversation_memory")\
                .select("memory_value")\
                .eq("memory_type", "image_context")\
                .eq("memory_key", f"images_{session_id}")\
                .order("created_at", desc=True)\
                .limit(5)\
                .execute()
            
            images = []
            for record in memory_result.data or []:
                memory_data = record.get("memory_value", {})
                images.extend(memory_data.get("analysis_summaries", []))
            
            return images
        except Exception as e:
            logger.warning(f"Could not get recent image context: {e}")
            return []
    ...
    async def process_message(self, request: UnifiedIrisRequest) -> UnifiedIrisResponse:
        ...
        # DEBUG: ENTRY POINT LOGGING - UPDATED CODE VERSION 2.0
        logger.info(f"🚀🚀🚀 NEW CODE LOADED - PROCESS_MESSAGE ENTRY v2.0:")
        logger.info(f"   - Message: '{request.message}'")
        logger.info(f"   - Images: {len(request.images) if request.images else 0}")
        logger.info(f"   - User ID: {request.user_id}")
        logger.info(f"   - This message confirms our updated code is loaded!")
        ...
        try:
            ...
            # DEBUG: Log every request with full details
            logger.info(f"🚀 IRIS Processing Request:")
            logger.info(f"   - message: '{request.message}'")
            logger.info(f"   - images: {len(request.images) if request.images else 0}")
            logger.info(f"   - request.message type: {type(request.message)}")
            logger.info(f"   - request.images type: {type(request.images)}")
            logger.info(f"   - request.images value: {request.images}")
            
            # DIRECT PHOTO STORAGE HANDLER - Highest Priority (Before workflows interfere)
            logger.info(f"🎯 CHECKING STORAGE CONDITIONS:")
            logger.info(f"   - request.message exists: {bool(request.message)}")
            logger.info(f"   - request.images exists: {bool(request.images)}")
            logger.info(f"   - images length > 0: {len(request.images) > 0 if request.images else False}")
            
            if request.message and request.images and len(request.images) > 0:
                message_lower = request.message.lower()
                storage_triggers = ["add this", "add it", "save this", "save it", "store this", "store it", "put this", "put it"]
                confirmation_triggers = ["ok do it", "ok please", "yes do", "do it", "please do", "go ahead"]
                
                is_storage_request = any(trigger in message_lower for trigger in storage_triggers + confirmation_triggers)
                
                if is_storage_request:
                    logger.info(f"🎯 DIRECT STORAGE REQUEST DETECTED: {request.message}")
                    logger.info(f"📸 Processing {len(request.images)} images for direct storage")
                    ...
                    try:
                        ...
                        # Look up room_id for the detected room type
                        room_id = await self._get_room_id_for_user(request.user_id, room_type)
                        ...
                        for img in request.images:
                            photo_id = self._store_to_property_photos(
                                ...
                                filename=img.get("filename", f"photo_{int(datetime.now().timestamp())}.png"),
                                session_id=session_id,
                                room_id=room_id  # Use the actual room_id from database
                            )
                            ...
                            if photo_id:
                                ...
                                logger.info(f"✅ Successfully stored photo {photo_id}")
                        
                        # Create truthful response based on actual results
                        if stored_photos:
                            room_text = f" in your {room_type}" if room_type else ""
                            response_text = f"✅ Successfully saved {len(stored_photos)} photo(s) to your property{room_text}!"
                            logger.info(f"🎉 STORAGE SUCCESS: {len(stored_photos)} photos saved")
                        else:
                            response_text = "❌ Failed to save photos. Please try again."
                            logger.error("💥 STORAGE FAILED: No photos were saved")
                        
                        # Save conversation and return immediately (bypass all other processing)
                        ...
                        return UnifiedIrisResponse(
                            ...
                            suggestions=["Upload another photo", "View property photos", "Create bid card"],
                            session_id=session_id,
                            reasoning=reasoning,
                            available_tools=[],
                            context_summary={
                                "inspiration_boards": 0,
                                "property_photos": len(stored_photos),
                                "trade_projects": 0
                            },
                            workflow_questions=[]
                        )
                        ...
                        logger.error(f"💥 Direct storage failed: {e}")
                        error_response = f"Failed to save photos: {str(e)}"
                        await self._save_conversation_message(session_id, request.user_id, request.message, error_response)
            ...
            if request.trigger_image_workflow and request.images:
                ...
                # Store images in session memory for later retrieval
                memory_key = f"pending_images_{session_id}"
                image_data_to_store = {
                ...
                db.client.table("unified_conversation_memory").upsert({
                    ...
                    "memory_value": json.dumps(image_data_to_store),
                    "created_at": datetime.now().isoformat(),
                    "updated_at": datetime.now().isoformat()
                }).execute()
                logger.info(f"Stored {len(request.images)} images in session memory with key {memory_key}")
                
                # Skip separate Vision API call - Claude will analyze images directly
                # Generate first workflow question for image classification
                workflow_questions = [
                    {
                ...
                logger.info(f"DEBUG: Immediately after setting workflow_questions, len = {len(workflow_questions)}")
            ...
            if request.workflow_response:
                ...
                # Retrieve images from session memory if not provided
                stored_images = request.images
                if not stored_images or len(stored_images) == 0:
                    # Try to retrieve from memory
                    memory_key = f"pending_images_{session_id}"
                    memory_result = db.client.table("unified_conversation_memory").select("memory_value").eq("tenant_id", request.user_id).eq("memory_key", memory_key).eq("memory_type", "session_data").limit(1).execute()
                    if memory_result.data and memory_result.data[0]:
                        stored_images_data = json.loads(memory_result.data[0]["memory_value"])
                        stored_images = stored_images_data.get("images", [])
                        logger.info(f"Retrieved {len(stored_images)} images from session memory")
                
                # Save updated image context with workflow response
                await self._save_image_context_to_memory(request.user_id, session_id, {"total_images": len(stored_images) if stored_images else 0, "results": []}, request.workflow_response)
                
                # Check if we need to ask the second question (room selection)
                if question_index == 0 and selected_option in ["Property Photos", "Both"]:
                    # Save workflow state for the second question
                    await self._save_workflow_state(request.user_id, session_id, {
                        "storage_location": selected_option,
                        "images": stored_images,
                        "pending_room_selection": True
                    })
                    
                    # User wants to store to property photos, ask for room selection
                    workflow_questions = [
                        {
                            "question": "What room or area is this photo of?",
                            "options": ["Backyard", "Kitchen", "Bathroom", "Living Room", "Bedroom", "Other"],
                            "callback": "identify_room_type"
                        }
                    ]
                    logger.info(f"Generated second workflow question for room selection and saved workflow state")
                else:
                    # Handle actual image storage based on user's choice
                    await self._handle_image_storage(request.user_id, session_id, request.workflow_response, stored_images)
            
            # LEGACY WORKFLOW DETECTION - DISABLED to prevent interference with direct storage
            elif False:  # Disabled: not request.workflow_response and request.images and len(request.images) > 0 and not request.trigger_image_workflow:
            ...
            # Also handle confirmations like "OK do it" when we have images in context
            has_storage_request = request.message and any(phrase in request.message.lower() for phrase in [
                "add it", "add this", "add to", "save it", "save to", "store", "put it",
                "ok do it", "ok please", "yes do", "do it", "please do", "go ahead"
            ])
            
            # Check if we have images in current request
            has_images = request.images and len(request.images) > 0
            
            if has_storage_request and has_images:
                ...
                for img in request.images:
                    try:
                        ...
                        # Note: _store_to_property_photos is not async, can call directly
                        photo_id = self._store_to_property_photos(
                            user_id=request.user_id,
                            image_data=img.get("data"),
                            filename=img.get("filename", "uploaded_photo.png"),
                            session_id=session_id,
                            room_id=None  # Room assignment handled separately for now
                        )
                        ...
                        if photo_id:
                            ...
                            action_results.append({
                                ...
                                "message": f"Photo saved to {room_type or 'property documentation'}"
                            })
                    except Exception as e:
                ...
                # Set response based on actual storage result
                if stored_images:
                    response_text = f"Successfully added {len(stored_images)} photo(s) to your {room_type or 'property documentation'}."
                    if room_type:
                        response_text += f" Filed under: {room_type}."
                    
                    # Skip Claude API since we have a definitive response
                    suggestions = ["Upload another photo", "View property photos", "Create a bid card"]
                    ...
                    return UnifiedIrisResponse(
                        ...
                        context_summary={
                            ...
                            "property_photos": len(context.get("property", {}).get("property_photos", [])) + len(stored_images),
    ...
    def _generate_anthropic_response(
        ...
        try:
            ...
            # Call Anthropic with Claude Sonnet
    ...
    async def _save_image_context_to_memory(self, user_id: str, session_id: str, image_data: Dict[str, Any], workflow_response: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            memory_insert = {
                "tenant_id": user_id,  # Use tenant_id instead of user_id
    ...
    async def _handle_image_storage(self, user_id: str, session_id: str, workflow_response: Dict[str, Any], images: Optional[List[Dict[str, Any]]]):
        ...
        try:
            ...
            # Options: ["Inspiration Board", "Property Photos", "Both"]
            if question_index == 0:
                # Store the first response and trigger room selection for property photos
                await self._save_workflow_state(user_id, session_id, {
                    "storage_location": selected_option,
                    "images": images,
                    "pending_room_selection": selected_option in ["Property Photos", "Both"]
                })
                
                # If only inspiration board, store immediately
                if selected_option == "Inspiration Board":
                    logger.info(f"Processing {len(images)} images for inspiration board only")
                    for i, img in enumerate(images):
                        image_data = img.get('data', '')
                        filename = img.get('filename', f'uploaded_image_{i}.jpg')
                        logger.info(f"Storing image {i} to inspiration board")
                        self._store_to_inspiration_board(user_id, image_data, filename, session_id)
                    
                    logger.info(f"Successfully stored {len(images)} images to inspiration board")
                
                # If property photos are involved, room selection is needed - don't store yet
                logger.info(f"Workflow state saved, awaiting room selection for: {selected_option}")
            
            # Question 1 is "What room or area is this?" 
            # Options: ["Backyard", "Kitchen", "Bathroom", "Living Room", "Bedroom", "Other"]
            elif question_index == 1:
                # Get the saved workflow state
                workflow_state = await self._get_workflow_state(user_id, session_id)
                if not workflow_state:
                    logger.error("No workflow state found for question 1")
                    return
                
                storage_location = workflow_state.get("storage_location", "")
                saved_images = workflow_state.get("images", images)
                
                logger.info(f"Processing {len(saved_images)} images for final storage - location: {storage_location}, room: {selected_option}")
                
                # Map room names to actual room IDs from the database
                room_id = await self._get_room_id_by_name(user_id, selected_option)
                
                for i, img in enumerate(saved_images):
                    image_data = img.get('data', '')
                    filename = img.get('filename', f'uploaded_image_{i}.jpg')
                    
                    # Store to inspiration board if needed
                    if storage_location in ["Inspiration Board", "Both"]:
                        logger.info(f"Storing image {i} to inspiration board")
                        self._store_to_inspiration_board(user_id, image_data, filename, session_id)
                    
                    # Store to property photos if needed (with room assignment)
                    if storage_location in ["Property Photos", "Both"]:
                        logger.info(f"Storing image {i} to property photos with room_id: {room_id}")
                        self._store_to_property_photos(user_id, image_data, filename, session_id, room_id)
                
                # Clear the workflow state
                await self._clear_workflow_state(user_id, session_id)
                logger.info(f"Successfully stored {len(saved_images)} images to {storage_location} in room: {selected_option}")
            
            else:
                logger.warning(f"Unknown question_index {question_index} - expected 0 or 1")
                    
        except Exception as e:
            logger.error(f"Failed to store images: {e}", exc_info=True)
    
    def _store_to_inspiration_board(self, user_id: str, image_data: str, filename: str, session_id: str):
        """Store image to inspiration board - simplified version without bucket storage"""
        try:
            import uuid
            from datetime import datetime
            from database_simple import db
            
            logger.info(f"Starting _store_to_inspiration_board for user {user_id}, filename {filename}")
            
            # First, ensure user has an inspiration board
            board_result = db.client.table("inspiration_boards").select("id").eq("user_id", user_id).limit(1).execute()
            
            board_id = None
            if board_result.data:
                board_id = board_result.data[0]["id"]
                logger.info(f"Found existing inspiration board: {board_id}")
            else:
                ...
                board_create_result = db.client.table("inspiration_boards").insert(new_board).execute()
                board_id = new_board["id"]
                logger.info(f"Created new inspiration board: {board_id}")
            
            # For now, store base64 directly without bucket storage
            image_entry = {
                "id": str(uuid.uuid4()),
                "board_id": board_id,
                "user_id": user_id,
                "image_url": image_data,  # Store base64 directly
                "thumbnail_url": None,
                "source": "upload",
                "source_url": None,
                "tags": ["iris", "uploaded"],
                ...
                "position": 0,
                "created_at": datetime.now().isoformat(),
                "category": "ideal"  # Changed from "uploaded" to "ideal" to match database constraint
            }
            
            result = db.client.table("inspiration_images").insert(image_entry).execute()
            logger.info(f"Successfully stored image to inspiration_images table: {image_entry['id']}")
            logger.info(f"Database insert result: {result}")
            
            return image_entry["id"]
            
        except Exception as e:
            logger.error(f"Failed to store to inspiration board: {e}", exc_info=True)
    
    def _store_to_property_photos(self, user_id: str, image_data: str, filename: str, session_id: str, room_id: str = None):
        """Store image to property photos - simplified version without bucket storage"""
        try:
            import uuid
            from datetime import datetime
            from database_simple import db
            
            logger.info(f"Starting _store_to_property_photos for user {user_id}, filename {filename}, room_id {room_id}")
            ...
            if property_result.data:
                ...
                logger.info(f"Found existing property: {property_id}")
            else:
                ...
                logger.info(f"Created new property: {property_id}")
            
            # Generate appropriate AI description based on room
            room_descriptions = {
                "Backyard": "Backyard photo uploaded via IRIS assistant",
                "Kitchen": "Kitchen photo uploaded via IRIS assistant", 
                "Bathroom": "Bathroom photo uploaded via IRIS assistant",
                "Living Room": "Living room photo uploaded via IRIS assistant",
                "Bedroom": "Bedroom photo uploaded via IRIS assistant",
                "Other": "Property photo uploaded via IRIS assistant"
            }
            
            room_name = "Unknown"
            if room_id:
                # Try to get room name from database
                try:
                    room_result = db.client.table("property_rooms").select("name").eq("id", room_id).limit(1).execute()
                    if room_result.data:
                        room_name = room_result.data[0]["name"]
                except Exception as e:
                    logger.warning(f"Could not get room name for {room_id}: {e}")
            
            ai_description = room_descriptions.get(room_name, f"{room_name} photo uploaded via IRIS assistant")
            
            # For now, just save the base64 data directly as the photo_url
            # This is a temporary fix - ideally should use bucket storage
            photo_entry = {
                "id": str(uuid.uuid4()),
                "property_id": property_id,
                "room_id": room_id,  # Now properly assigned from room selection
                "photo_url": image_data,  # Store base64 directly for now
                "original_filename": filename,
                ...
                "ai_classification": {
                    ...
                    "session_id": session_id,
                    "upload_timestamp": datetime.now().isoformat(),
                    "classification": f"{room_name.lower().replace(' ', '_')}_documentation",
                    "area": room_name.lower().replace(' ', '_'),
            ...
            logger.info(f"Successfully stored image to property_photos table: {photo_entry['id']} with room_id: {room_id}")
            logger.info(f"Database insert result: {result}")
            
            return photo_entry["id"]
            
        except Exception as e:
            logger.error(f"Failed to store to property photos: {e}", exc_info=True)
    
    async def _save_workflow_state(self, user_id: str, session_id: str, state_data: Dict[str, Any]):
        """Save workflow state to memory for multi-step workflow"""
        try:
            import json
            import uuid
            from datetime import datetime
            ...
            # Store workflow state in unified memory
            db.client.table("unified_conversation_memory").insert({
                "id": str(uuid.uuid4()),
                "tenant_id": user_id,
                "memory_key": memory_key,
                "memory_type": "workflow_state",
                "memory_value": json.dumps(state_data),
                "created_at": datetime.now().isoformat(),
                "updated_at": datetime.now().isoformat()
            }).execute()
            
            logger.info(f"Saved workflow state with key {memory_key}: {state_data}")
            
        except Exception as e:
            logger.error(f"Failed to save workflow state: {e}", exc_info=True)
    
    async def _get_workflow_state(self, user_id: str, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = db.client.table("unified_conversation_memory").select("memory_value").eq("tenant_id", user_id).eq("memory_key", memory_key).limit(1).execute()
            
            if result.data:
                state_data = json.loads(result.data[0]["memory_value"])
                logger.info(f"Retrieved workflow state: {state_data}")
                return state_data
            else:
                logger.warning(f"No workflow state found for key {memory_key}")
                return {}
    ...
    async def _clear_workflow_state(self, user_id: str, session_id: str):
        ...
        try:
            ...
            # Delete the workflow state record
            db.client.table("unified_conversation_memory").delete().eq("tenant_id", user_id).eq("memory_key", memory_key).execute()
            
            logger.info(f"Cleared workflow state with key {memory_key}")
            
        except Exception as e:
            logger.error(f"Failed to clear workflow state: {e}", exc_info=True)
    
    async def _get_room_id_by_name(self, user_id: str, room_name: str) -> str:
        """Get room ID by room name, creating default rooms if needed"""
        try:
            from database_simple import db
            import uuid
            from datetime import datetime
            
            logger.info(f"Looking up room ID for room name: {room_name}")
            
            # First, get the user's property
            property_result = db.client.table("properties").select("id").eq("user_id", user_id).limit(1).execute()
            
            if not property_result.data:
                logger.error(f"No property found for user {user_id}")
                return None
            
            property_id = property_result.data[0]["id"]
            
            # Map room names to room types
            room_type_map = {
                "Backyard": "backyard",
                "Kitchen": "kitchen", 
                "Bathroom": "bathroom",
            ...
            room_type = room_type_map.get(room_name, "other")
            
            # Look for existing room
            room_result = db.client.table("property_rooms").select("id").eq("property_id", property_id).eq("name", room_name).limit(1).execute()
            ...
            new_room = {
                ...
                "name": room_name,
                "room_type": room_type,
                "description": f"{room_name} created by IRIS photo upload",
                "created_at": datetime.now().isoformat()
            }
            
            create_result = db.client.table("property_rooms").insert(new_room).execute()
            room_id = new_room["id"]
            ...
            logger.error(f"Failed to get/create room ID for {room_name}: {e}", exc_info=True)
            return None
    ...
    def _detect_room_from_message(self, message: str) -> str:
        ...
        try:
            ...
            room_keywords = {
                ...
                "bedroom": ["bedroom", "bed room", "master bedroom", "sleep"],
                "living room": ["living room", "family room", "lounge", "great room"],
                "dining room": ["dining room", "dining area", "eat"],
                "basement": ["basement", "cellar", "downstairs"],
                "attic": ["attic", "loft", "upstairs"],
                "garage": ["garage", "car port", "driveway"],
                "exterior": ["exterior", "outside", "roof", "siding", "front"],
    ...
    async def _get_room_id_for_user(self, user_id: str, room_type: str) -> str:
        ...
        try:
            ...
            property_id = property_result.data[0]["id"]
            logger.info(f"Found property_id: {property_id} for user: {user_id}")
            
            # Look for room with matching room_type in this property
            ...
            if room_result.data:
                room_id = room_result.data[0]["id"]
                room_name = room_result.data[0]["name"]
                logger.info(f"Found existing room: {room_name} (ID: {room_id}) for room_type: {room_type}")
                return room_id
            else:
                # Try to find by name if room_type didn't match
                room_name_variants = {
                    "backyard": ["backyard", "back yard", "yard"],
                    "kitchen": ["kitchen"],
                ...
                # Check all rooms for name matches
                all_rooms_result = db.client.table("property_rooms").select("id, name, room_type").eq("property_id", property_id).execute()
                
                if all_rooms_result.data:
                    for room in all_rooms_result.data:
                        room_name_lower = room["name"].lower()
                        variants = room_name_variants.get(room_type, [room_type])
                        
                        if any(variant in room_name_lower for variant in variants):
                            logger.info(f"Found room by name match: {room['name']} (ID: {room['id']}) for room_type: {room_type}")
                            return room["id"]
                
                logger.warning(f"No room found for room_type: {room_type} in property: {property_id}")
                return None
                
        except Exception as e:
            logger.error(f"Error looking up room_id for user {user_id}, room_type {room_type}: {e}")
            return None
