"""
Database Manager Tool - Copied from COIA
...
class DatabaseManagerTool:
    ...
    async def save_potential_contractor(self, profile: Dict[str, Any]) -> Dict[str, Any]:
        ...
        This does NOT promote to contractors table.
        """
        try:
            ...
            if contractor_size_category not in valid_contractor_sizes:
                contractor_size_category = None

            # Build staging payload
            staging_data: Dict[str, Any] = {
                "id": staging_id,
                "contractor_lead_id": profile.get("contractor_lead_id"),
                ...
                "specialties": profile.get("specialties", []) or profile.get("specializations", []),
                ...
                # CRITICAL: Add contractor_type_ids for BSA bid matching
    ...
    async def check_existing_contractor(self, company_name: str) -> Optional[Dict[str, Any]]:
        """
        Check if contractor already exists in contractors or potential_contractors table
        """
        try:
            sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))))
            from database_simple import get_client
            ...
            if result and result.data:
                ...
                contractor["is_full_contractor"] = True
                return contractor
            
            # Then check potential_contractors table
            result = supabase.table("potential_contractors").select("*").ilike("company_name", f"%{company_name}%").execute()
            
            if result and result.data:
                logger.info(f"Found existing contractor in potential_contractors table: {company_name}")
                potential = result.data[0]
                potential["table"] = "potential_contractors"
                potential["is_full_contractor"] = False
                return potential
