...
class MCPEmailChannelWithOpenAI:
    """Email channel that uses GPT-4 to write personalized emails"""
    ...
    def __init__(self):
        ...
        if not self.openai_api_key:
            print("[MCPEmailChannel+GPT4 ERROR] OPENAI_API_KEY not found in environment!")
        else:
            print(f"[MCPEmailChannel+GPT4] Found API key: {self.openai_api_key[:10]}...")

        self.client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
        self.sent_emails = []  # For testing verification
        print("[MCPEmailChannel+GPT4] Initialized with GPT-4 integration")
