...
class ContextBuilder:
    ...
    def build_complete_context(
        ...
        context = ContextResponse(
            ...
            property_tasks=self._get_property_tasks(user_id),  # ADD PROPERTY TASKS
    ...
    def _get_property_tasks(self, user_id: str) -> List[Dict[str, Any]]:
        """Get all property tasks for the user - FIXED TO ACTUALLY WORK"""
        if not self.db:
            return []
        
        try:
            # DIRECTLY query property_tasks table
            result = self.db.client.table("property_tasks").select(
                "id, room_id, task_type, description, severity, status, estimated_cost, created_at"
            ...
            for task in result.data or []:
                ...
                room_name = None
                if task.get("room_id"):
                    room_result = self.db.client.table("property_rooms").select(
                        "name"
                    ).eq("id", task["room_id"]).single().execute()
                    
                    if room_result.data:
                        room_name = room_result.data.get("name")
                
                tasks.append({
                    "task_id": task.get("id"),
                    "room_id": task.get("room_id"),
                    "room_name": room_name or "Property",
                    "type": task.get("task_type"),
                    "description": task.get("description"),
                    "priority": task.get("severity"),  # severity column, not priority
    ...
    def _get_previous_session_conversations(self, user_id: str) -> str:
        """Get summary of previous session conversations for the user"""
        if not self.db:
            return ""
        ...
        try:
            ...
            # Query for session_conversation type memories
            result = self.db.client.table("unified_conversation_memory").select(
                "memory_value, created_at"
            ).in_("conversation_id", conv_ids).eq("memory_type", "session_conversation").order(
            ...
            # Extract key information from previous sessions
            session_insights = []
            for memory in result.data:
                memory_value = memory.get("memory_value", {})
                if isinstance(memory_value, dict):
                    user_input = memory_value.get("user_input", "")
                    # Extract key preferences and topics from previous conversations
                    if "kitchen" in user_input.lower():
                        session_insights.append("kitchen design interest")
                    if "bathroom" in user_input.lower():
                        session_insights.append("bathroom renovation interest")
                    if "navy" in user_input.lower() or "blue" in user_input.lower():
                        session_insights.append("likes navy blue")
                    if "modern" in user_input.lower():
                        session_insights.append("modern style preference")
                    if "farmhouse" in user_input.lower():
                        session_insights.append("farmhouse style preference")
                    if "budget" in user_input.lower():
                        # Try to extract budget info
                        import re
                        budget_match = re.search(r'\$?\d+[kK]?', user_input)
                        if budget_match:
                            session_insights.append(f"budget mentioned: {budget_match.group()}")
            
            # Return unique insights
            unique_insights = list(set(session_insights))
            if unique_insights:
                return ", ".join(unique_insights[:5])  # Return top 5 insights
            
            return ""
            
        except Exception as e:
            logger.error(f"Error getting previous session conversations: {e}")
            return ""
    ...
    def build_context_summary(self, user_id: str, conversation_id: str) -> str:
        ...
        # Load previous session conversations to get user history
        previous_session_data = self._get_previous_session_conversations(user_id)
        if previous_session_data:
            summary_parts.append(f"Previous interactions: {previous_session_data}")
        
        # Property tasks - SHOW ACTUAL TASKS
        if hasattr(context, 'property_tasks') and context.property_tasks:
            task_count = len(context.property_tasks)
            summary_parts.append(f"Has {task_count} property maintenance tasks")
            
            # List specific tasks with room names
            for task in context.property_tasks[:3]:  # Top 3 tasks
                room_name = task.get('room_name', 'property')
                task_type = task.get('type', 'maintenance')
                description = task.get('description', '')[:50]
                summary_parts.append(f"{task_type} in {room_name}: {description}")
        
        # Property projects
        if hasattr(context, 'property_projects') and context.property_projects:
            summary_parts.append(f"User has {len(context.property_projects)} property projects")
        ...
        if hasattr(context, 'contractor_preferences') and context.contractor_preferences:
            prefs = list(context.contractor_preferences.keys())[:3]  # Top 3 preferences
            summary_parts.append(f"Contractor preferences: {', '.join(prefs)}")
        
        # Previous projects
        if hasattr(context, 'previous_projects') and context.previous_projects:
            summary_parts.append(f"Has {len(context.previous_projects)} previous property projects")
    ...
    async def get_complete_property_context(self, user_id: str) -> Dict[str, Any]:
        """Get complete property context for conversational flow - ASYNC VERSION"""
        # Use a dummy conversation_id since we just need property data
        context = self.build_complete_context(user_id, "property_context_query")
        
        # Convert ContextResponse to dictionary format expected by conversational flow
        return {
            "property_projects": context.property_projects,
            "project_context": context.project_context,
            "contractor_preferences": context.contractor_preferences,
            "previous_projects": context.previous_projects,
            "conversations_from_other_agents": context.conversations_from_other_agents,
            "photos_from_unified_system": context.photos_from_unified_system,
            "property_tasks": context.property_tasks,
            "privacy_level": context.privacy_level,
            "property_stats": {
                "total_rooms": len(context.property_projects),  # Approximate
                "total_photos": len(context.photos_from_unified_system.get("project_photos", [])),
                "total_tasks": len(context.property_tasks),
                "total_bid_cards": len(context.project_context.get("bid_cards", []))
            }
        }