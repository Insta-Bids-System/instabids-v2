...
async def get_user_context(user_id: str, project_id: Optional[str] = None):
    """
    Get comprehensive user context for IRIS Property agent
    Includes property projects, project context, contractor preferences, etc.
    """
    try:
...
@router.get("/tasks/{property_id}")
async def get_property_tasks(property_id: str):
    """Get all tasks for a property"""
    try:
        logger.info(f"Getting tasks for property {property_id}")
        
        agent = get_iris_agent()
        task_manager = agent.task_manager if hasattr(agent, 'task_manager') else None
        
        if not task_manager:
            from ..services.task_manager import TaskManager
            task_manager = TaskManager()
        
        tasks = task_manager.get_tasks_by_property(property_id)
...
@router.get("/tasks/room/{room_id}")
async def get_room_tasks(room_id: str):
    """Get all tasks for a specific room"""
    try:
        logger.info(f"Getting tasks for room {room_id}")
        ...
        tasks = task_manager.get_tasks_by_room(room_id)
        ...
        logger.error(f"Error getting room tasks: {e}")
        raise HTTPException(status_code=500, detail=f"Error retrieving tasks: {str(e)}")

@router.get("/tasks/{property_id}/grouped")
async def get_grouped_tasks(property_id: str):
    """Get tasks grouped by contractor type"""
    try:
        logger.info(f"Getting grouped tasks for property {property_id}")
        ...
        grouped = task_manager.group_tasks_by_contractor(property_id)
        
        # Convert to serializable format
        result = {}
        for contractor_type, tasks in grouped.items():
            result[contractor_type] = [task.dict() for task in tasks]
        
        return {
            "success": True,
            "grouped_tasks": result,
            "contractor_types": list(grouped.keys())
        }
        
    except Exception as e:
        logger.error(f"Error getting grouped tasks: {e}")
        raise HTTPException(status_code=500, detail=f"Error grouping tasks: {str(e)}")

@router.post("/tasks/bundle")
async def bundle_tasks(request: Dict[str, Any]):
    """Bundle tasks into a group for bid card creation"""
    try:
        task_ids = request.get('task_ids', [])
        group_name = request.get('group_name', 'Task Bundle')
        
        logger.info(f"Bundling {len(task_ids)} tasks")
        ...
        group_id = task_manager.create_task_group(task_ids, group_name)
        ...
        logger.error(f"Error bundling tasks: {e}")
        raise HTTPException(status_code=500, detail=f"Error bundling tasks: {str(e)}")

# Repair item management endpoints (DEPRECATED - use tasks instead)
@router.post("/repair-items")
async def add_repair_item(request: RepairItemRequest):
    """Add repair item to potential bid card (DEPRECATED - use tasks)"""
    try:
...
# IRIS Property agent focuses on property issues and bid cards, not inspiration boards
# Inspiration functionality moved to iris_inspiration agent
